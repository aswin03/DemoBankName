	...
	public ModelAndView changeDebitCardPin(DebitCard debitCard, HttpSession httpSession) {
		...
		try
		{
			...
			if (customerId != null) {
				 ...
				 * framework
				 */
				DebitCardService debitCardService = (DebitCardService) ContextFactory
						.getContext().getBean("debitCardService");
				...
				/*
				 * getContext() method of ContextFactory class parses the
				 * SpringConfig class and using @ComponentScan, it scans the
				 * packages/classes for which the base package provided is
				 * 'com.edubank' then it returns the object of
				 * ApplicationContext ApplicationContext holds the objects
